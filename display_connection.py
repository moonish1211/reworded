...
import random
import csv
from brainflow.board_shim import BoardShim, BrainFlowInputParams, BoardIdsdgge
...
# Open a CSV file for appending EEG data.
...
for word, label in combined_list:
    ...
    for i, letter in enumerate(word):
        ...
        clock = core.Clock()
        # (Optional: Reset flash marker flags here if needed)
        while clock.getTime() < duration:
            t = clock.getTime()
            # Flicker logic: here, the letter is simply shown (modify if you want actual flicker)
            text_stim.text = letter
            square_frame.draw()
            text_stim.draw()
            # Time-lock: flash the marker at the very start of the character display.
    ...
    if current_count > previous_sample_count:
        # Get all data collected so far and then slice for new samples.
        ...
        # Append each sample (transposed so that each row is one sample) to the CSV file.
        for sample in iteration_data.T:
            csv_writer.writerow(sample.tolist())
        csv_file.flush()
